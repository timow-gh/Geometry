...
public:
  Lcs();
  Lcs(const LinAl::Vec3<T> &origin, const LinAl::Vec3<T> &x,
      const LinAl::Vec3<T> &y);
  Lcs(const LinAl::Vec3<T> &origin, const LinAl::Vec3<T> &x,
      const LinAl::Vec3<T> &y, const LinAl::Vec3<T> &z);
...
template <typename T>
Lcs<T>::Lcs()
    : m_origin(LinAl::Vec3<T>{0}), m_coordVec{LinAl::Vec3<T>{1, 0, 0},
                                              LinAl::Vec3<T>{0, 1, 0},
                                              LinAl::Vec3<T>{0, 0, 1}} {}

template <typename T>
Lcs<T>::Lcs(const LinAl::Vec3<T> &origin, const LinAl::Vec3<T> &x,
            const LinAl::Vec3<T> &y)
    : m_origin(origin) {
  m_coordVec[0] = x;
  m_coordVec[1] = y;
...
template <typename T> bool Lcs<T>::operator==(const Lcs &rhs) const {
...
template <typename T> const LinAl::Vec3<T> &Lcs<T>::getOrigin() const {
  return m_origin;
}

template <typename T> void Lcs<T>::setOrigin(const LinAl::Vec3<T> &origin) {
  m_origin = origin;
}
...
template <typename T>
LinAl::Vec3Array<T, 3>
Lcs<T>::projectVector(const LinAl::Vec3<T> &vector) const {
...
template <typename T> const LinAl::Vec3<T> &Lcs<T>::getXVector() const {
  return m_coordVec[0];
}
template <typename T> const LinAl::Vec3<T> &Lcs<T>::getYVector() const {
  return m_coordVec[1];
}
template <typename T> const LinAl::Vec3<T> &Lcs<T>::getZVector() const {
  return m_coordVec[2];
}

template <typename T> void Lcs<T>::normalizeCoordVectors() {
