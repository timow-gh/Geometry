...
namespace Geometry {
template <typename T, std::size_t Dim> class AABB {
public:
...
template <typename T, std::size_t Dim> LinAl::Vec<T, Dim> &AABB<T, Dim>::min() {
  return m_min;
}
...
template <typename T, std::size_t Dim> LinAl::Vec<T, Dim> &AABB<T, Dim>::max() {
  return m_max;
}
