...
template <typename T> const Halfedge<T> &Facet<T>::halfedge() const {
  return m_mesh->getHalfedges()[m_halfedgeIndex];
}
template <typename T> Halfedge<T> &Facet<T>::halfedge() {
  return m_mesh->m_halfedges[m_halfedgeIndex];
}

template <typename T> bool Facet<T>::operator==(const Facet &rhs) const {
  return m_halfedgeIndex == rhs.m_halfedgeIndex && m_mesh == rhs.m_mesh;
}
template <typename T> bool Facet<T>::operator!=(const Facet &rhs) const {
  return !(rhs == *this);
}
template <typename T> std::size_t Facet<T>::getHalfedgeIndex() const {
  return m_halfedgeIndex;
}
template <typename T> bool Facet<T>::isValid() const {
