...
template <typename T> class HalfedgeMesh {
...
template <typename T> LinAl::Vec3Vector<T> &HalfedgeMesh<T>::getVertexPoints() {
...
template <typename T> MeshPoints<T> &HalfedgeMesh<T>::getTriangleMesh() {
...
template <typename T> const xg::Guid &HalfedgeMesh<T>::getGuid() const {
  return m_guid;
}
...
void HalfedgeMesh<T>::addTriangle(const Triangle<T, 3> &triangle) {
  ...
  std::size_t halfedgeIndex = m_halfedges.size() == 0 ? 0 : m_halfedges.size();
  ...
  for (std::size_t i = halfedgeIndex; i < m_halfedges.size(); ++i) {
    ...
    auto previousHeIndex = i == halfedgeIndex ? m_halfedges.size() - 1 : i - 1;
