...
namespace Geometry {
template <typename T>
std::optional<T> calcIntersectionParameter(
    const LinAl::Vec<T, 3> &planeOrigin, const LinAl::Vec<T, 3> &planeNormal,
    const LinAl::Vec3<T> &lineOrigin, const LinAl::Vec3<T> &lineDir,
    T eps = Core::eps_traits<T>::value());
template <typename T>
constexpr std::optional<LinAl::Vec3<T>>
calcIntersection(const Plane<T> &plane, const Line3<T> &line,
                 T eps = Core::eps_traits<T>::value()) {
  ...
  if (auto paramD = calcIntersectionParameter(planeOrigin, planeNormal,
                                              lineOrigin, lineDir))
...
template <typename T>
constexpr std::optional<LinAl::Vec3<T>>
calcIntersection(const Plane<T> &plane, const Ray3<T> &ray,
                 T eps = Core::eps_traits<T>::value()) {
  const LinAl::Vec<T, 3> &planeOrigin = plane.getOrigin();
  const LinAl::Vec<T, 3> &planeNormal = plane.getNormal();
...
template <typename T>
constexpr std::optional<LinAl::Vec3<T>>
calcIntersection(const Plane<T> &plane, const Segment3<T> &seg,
                 T eps = Core::eps_traits<T>::value()) {
  const LinAl::Vec<T, 3> &planeOrigin = plane.getOrigin();
  const LinAl::Vec<T, 3> &planeNormal = plane.getNormal();
