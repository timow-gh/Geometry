...
template <typename T> struct SphereIntersection {
...
calcIntersection(const Sphere<T> &sphere, const Line3<T> &line,
...
template <typename T>
std::optional<SphereIntersection<T>>
calcIntersection(const Sphere<T> &sphere, const Ray3<T> &ray,
                 T eps = Core::eps_traits<T>::value()) {
  // See Schneider - Geometric Tools, Linear Components and Spheres
  const LinAl::Vec3<T> &sphereOrigin = sphere.getOrigin();
  const T radius = sphere.getRadius();
...
template <typename T>
std::optional<SphereIntersection<T>>
calcIntersection(const Sphere<T> &sphere, const Segment3<T> &segment,
                 T eps = Core::eps_traits<T>::value()) {
  // See Schneider - Geometric Tools, Linear Components and Spheres
  const LinAl::Vec3<T> &sphereOrigin = sphere.getOrigin();
  const T radius = sphere.getRadius();
