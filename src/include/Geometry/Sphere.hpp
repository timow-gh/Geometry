...
template <typename T> const LinAl::Vec3<T> &Sphere<T>::getOrigin() const {
  return m_origin;
}
template <typename T> T Sphere<T>::getRadius() const { return m_radius; }
template <typename T> bool Sphere<T>::operator==(const Sphere &rhs) const {
