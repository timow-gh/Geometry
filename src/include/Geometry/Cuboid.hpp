...
public:
  Cuboid(const LinAl::Vec3<T> &origin,
...
template <typename T>
Cuboid<T>::Cuboid(const LinAl::Vec3<T> &origin,
                  const LinAl::Vec3Array<T, 3> &sideVectors)
    : m_origin(origin), m_sideVectors(sideVectors) {}

template <typename T>
Cuboid<T>::Cuboid(const LinAl::Vec3<T> &origin, const LinAl::Vec3<T> &diagonal)
    : m_origin(origin) {
...
template <typename T>
Cuboid<T>::Cuboid(const LinAl::Vec3<T> &diagonal)
    : Cuboid(LinAl::Vec3<T>{0, 0, 0}, diagonal) {}
template <typename T> const LinAl::Vec3<T> &Cuboid<T>::origin() const {
  return m_origin;
}
template <typename T>
...
template <typename T> bool Cuboid<T>::operator==(const Cuboid &rhs) const {
