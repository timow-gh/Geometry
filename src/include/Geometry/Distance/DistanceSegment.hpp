...
constexpr T distance(const Segment<T, D>& segment, const LinAl::Vec<T, D>& vec)
{
...
template <typename T, std::size_t D>
constexpr T distance(const LinAl::Vec<T, D>& vec, const Segment<T, D>& segment)
{
    return distance(segment, vec);
}

template <typename T, std::size_t D>
constexpr T distance(const Segment<T, D>& segment, const Line<T, D>& line)
{
    assert(false);
    return 0;
}
...
template <typename T, std::size_t D>
constexpr T distance(const Line<T, D>& line, const Segment<T, D>& segment)
{
    return distance(segment, line);
}

template <typename T, std::size_t D>
constexpr T distance(const Segment<T, D>& lhs, const Segment<T, D>& rhs)
{
    assert(false);
    return 0;
}
