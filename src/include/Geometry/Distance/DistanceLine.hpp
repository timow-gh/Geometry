...
template <typename T, std::size_t D>
[[nodiscard]] constexpr T distance(const Line<T, D>& line,
...
template <typename T, std::size_t D>
[[nodiscard]] constexpr T distance(const Line<T, D>& lhs, const Line<T, D>& rhs)
{
    LinAl::Vec<T, D> cross =
    ...
    return LinAl::dot(LinAl::Vec<T, D>{lhs.getOrigin() - rhs.getOrigin()},
                      cross) /
           crossLen;
}
