...
std::unique_ptr<HalfedgeMesh<T>> buildCuboidMesh(const Cuboid<T> &cube) {
  ...
  calcCuboidFaceTriangles(triangles, {defaultOrigin, z, diag, y}, cube.origin(),
                          x, 0);

  diag = x + z;
  calcCuboidFaceTriangles(triangles, {defaultOrigin, x, diag, z}, cube.origin(),
                          y, 4);

  diag = x + y;
  calcCuboidFaceTriangles(triangles, {defaultOrigin, y, diag, x}, cube.origin(),
                          z, 8);
