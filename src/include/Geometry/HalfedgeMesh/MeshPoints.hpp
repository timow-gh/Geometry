...
template <typename T> class MeshPoints {
public:
  MeshPoints() = default;
  ...
  [[nodiscard]] bool contains(const LinAl::Vec3<T> &vector, std::size_t &index);
...
template <typename T> std::size_t MeshPoints<T>::size() const {
...
template <typename T> LinAl::Vec3Vector<T> &MeshPoints<T>::getPoints() {
  return m_points;
}
