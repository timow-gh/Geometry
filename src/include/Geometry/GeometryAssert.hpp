...
#define GEOMETRY_PRECONDITION_SPHERE_ASSERT(sphere)                                                \
    CORE_PRECONDITION_ASSERT(!Core::isZero(sphere.getRadius()), "Sphere with radius zero.")
...
#define GEOMETRY_PRECONDITION_RAY_DIRECTION_ASSERT(ray)                                            \
    CORE_PRECONDITION_ASSERT(!Core::isZero(LinAl::norm2Squared(ray.getDirection())),               \
...
#else
#define GEOMETRY_PRECONDITION_RAY_DIRECTION_DEBUG_ASSERT(ray)                                      \
    CORE_PRECONDITION_DEBUG_ASSERT(!Core::isZero(LinAl::norm2Squared(ray.getDirection())),         \
                                   "Direction with zero length is not allowed.")
#endif
