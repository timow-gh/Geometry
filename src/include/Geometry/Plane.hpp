...
template <typename T> const LinAl::Vec3<T> &Plane<T>::getOrigin() const {
  return m_origin;
}
template <typename T> const LinAl::Vec3<T> &Plane<T>::getNormal() const {
  return m_normal;
}
template <typename T> T Plane<T>::distance(const LinAl::Vec3<T> &vec) const {
